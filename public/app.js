...
- Frame analysis as insight, not financial advice`,
    ...
    function WatchlistTab({ C, watchlist, setWatchlist }) {
      const [quotes, setQuotes] = useState({});
      const [loading, setLoading] = useState(false);
      const refresh = async () => {
          ...
          try {
            ...
            if (q.c > 0) {
              ...
              try {
                ...
                if (md.metric) { fetched[sym].wHigh = md.metric['52WeekHigh'] || q.h; fetched[sym].wLow = md.metric['52WeekLow'] || q.l; }
              } catch {}
            }
          } catch {}
        }));
              ...
              {watchlist.map(sym => {
                ...
                return (
                  <div key={sym} style={{ background:C.bgCard, border:'1px solid '+C.border, borderRadius:12, padding:'16px 20px' }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom: q ? 12 : 0 }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                        {q && <div style={{ textAlign:'right' }}>
                          ...
                          <div style={{ fontSize:11, color:isUp?C.green:C.red }}>{isUp?'▲':'▼'} {Math.abs(q.change).toFixed(2)}%</div>
                        </div>}
                    ...
                    {q && <div style={{ display:'flex', gap:16, paddingTop:10, borderTop:'1px solid '+C.border }}>
                      {[['DAY H', q.high, C.green],['DAY L', q.low, C.red],['52W H', q.wHigh, C.green],['52W L', q.wLow, C.red]].map(([label, val, color]) => (
                        <div key={label} style={{ textAlign:'center' }}>
                          <div style={{ fontSize:9, color:C.textMuted, letterSpacing:'0.05em', marginBottom:2 }}>{label}</div>
                          <div style={{ fontSize:12, color:color, fontFamily:"'DM Mono',monospace", fontWeight:600 }}>{val ? ' = [
          ...
          <div style={{ display:"flex", paddingTop:96, minHeight:"100vh", position:"relative", zIndex:1 }}>
            ...
            <main style={{ flex:1, marginLeft:"clamp(0px, 220px, 220px)", padding:"32px 40px", minHeight:"calc(100vh - 60px)", maxWidth:"calc(100vw - 220px)" }} className="main-content">
              <div key={tab} style={{ animation:"fadeIn 0.2s both", maxWidth:1100 }}>
                ...
                {tab==="watchlist" && <WatchlistTab C={C} watchlist={watchlist} setWatchlist={setWatchlist}/>}
