...
- Frame analysis as insight, not financial advice`,
                ...
                <div key={key} style={{ background:C.bgCard, border:`1px solid ${isAdded?C.green+"60":isOpen?C.cyan+"44":C.border}`, borderRadius:14, padding:"12px 15px", animation:`fadeIn 0.15s ${Math.min(i,8)*0.04}s both`, transition:"border-color 0.2s" }}>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:10, marginLeft:10, flexShrink:0 }}>
                      ...
                      {(() => { const isW=(watchlist||[]).includes(item.symbol); return <button onMouseDown={e=>e.preventDefault()} onClick={()=>isW?setWatchlist(prev=>prev.filter(s=>s!==item.symbol)):setWatchlist(prev=>[...prev,item.symbol])} style={{background:isW?'rgba(167,139,250,0.15)':'none',border:'1px solid '+(isW?'#a78bfa':'#1a2840'),borderRadius:8,padding:'6px 10px',color:isW?'#a78bfa':'#5a7090',fontSize:10,fontWeight:700,fontFamily:"'Space Mono',monospace",cursor:'pointer',marginRight:6}}>{isW?'👁 WATCHING':'👁 WATCH'}</button>; })()}
      {/* Add button */}
                      {isAdded ? (
      ...
      const refresh = async () => {
        ...
        await Promise.allSettled(watchlist.map(async sym => {
          try {
            ...
            if (q.c > 0) {
              fetched[sym] = { price: q.c, change: q.dp||0, high: q.h||0, low: q.l||0, wHigh: q.h||0, wLow: q.l||0 };
              try {
                const m = await fetch('/api/finnhub?endpoint=metric&symbol='+encodeURIComponent(sym));
                const md = await m.json();
                if (md.metric) { fetched[sym].wHigh = md.metric['52WeekHigh']||q.h||0; fetched[sym].wLow = md.metric['52WeekLow']||q.l||0; }
              } catch {}
            }
          } catch {}
        }));
        setQuotes(fetched); setLoading(false);
      };
      ...
      return (
        <div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
            ...
            <button onClick={refresh} disabled={loading} style={{background:'none',border:'1px solid '+C.border,borderRadius:6,padding:'5px 12px',color:C.textMuted,fontSize:9,fontFamily:"'Space Mono',monospace",cursor:'pointer'}}>{loading?'···':'↻ REFRESH'}</button>
          </div>
          {watchlist.length===0 ? (
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:12}}>
                        <div style={{textAlign:'right'}}>
                          <div style={{fontWeight:700,color:C.text,fontSize:18,fontFamily:"'DM Mono',monospace"}}>{q?' = [
    ...
    root.render(<Stocker/>);+q.price.toFixed(2):<span style={{color:C.textDim,fontSize:13}}>Loading...</span>}</div>
                          {q && <div style={{fontSize:12,color:isUp?C.green:C.red}}>{isUp?'▲':'▼'} {Math.abs(q.change).toFixed(2)}%</div>}
                        </div>
                        <button onClick={()=>setWatchlist(p=>p.filter(s=>s!==sym))} style={{background:'none',border:'1px solid '+C.border,borderRadius:6,padding:'6px 10px',color:C.textMuted,fontSize:11,cursor:'pointer'}}>✕</button>
                      </div>
                    </div>
                    {q && (
                      <div style={{display:'flex',gap:0,borderTop:'1px solid '+C.border,paddingTop:10}}>
                        {[['DAY H',q.high,'green'],['DAY L',q.low,'red'],['52W H',q.wHigh,'green'],['52W L',q.wLow,'red']].map(([label,val,clr])=>(
                          <div key={label} style={{flex:1,textAlign:'center'}}>
                            <div style={{fontSize:9,color:C.textMuted,letterSpacing:'0.05em',marginBottom:3}}>{label}</div>
                            <div style={{fontSize:12,fontWeight:600,color:clr==='green'?C.green:C.red,fontFamily:"'DM Mono',monospace"}}>{val?' = [
      {sym:"BINANCE:BTCUSDT", l:"BTC"},
      {sym:"BINANCE:ETHUSD",  l:"ETH"},
      {sym:"BINANCE:SOLUSD",  l:"SOL"},
      {sym:"AAPL",  l:"AAPL"},
    ...
    const root = ReactDOM.createRoot(document.getElementById("root"));
    root.render(<Stocker/>);+val.toFixed(2):'—'}</div>
                          </div>
                        ))}
                      </div>
                    )}
