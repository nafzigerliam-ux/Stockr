...
- Frame analysis as insight, not financial advice`,
    ...
    function Stocker() {
      ...
      useEffect(() => {
        _supabase.auth.getSession().then(({data:{session}})=>{ if(!session?.user?.id) return; const uid=session.user.id; try{localStorage.setItem("stocker_portfolio_"+uid,JSON.stringify(portfolio));}catch{} _supabase.from("portfolios").upsert({user_id:uid,holdings:portfolio,updated_at:new Date().toISOString()},{onConflict:"user_id"}); });
      }, [portfolio]);
